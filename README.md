...
**Действие 1.** Начало транзакции. Система фиксирует начало операции.

**Действие 2.** Проверка и блокировка счёта пользователя. Система проверяет существование счёта и его текущее состояние (не заблокирован / активен). На запись накладывается блокировка, чтобы предотвратить одновременное изменение баланса другими операциями (например, другим пополнением или списанием).

**Действие 3.** Списание средств с платёжного инструмента. Если списание не удалось, транзакция откатывается.

**Действие 4.** Зачисление средств на счёт телефона. После успешного списания сумма прибавляется к текущему балансу телефона.

**Действие 5.** Фиксация транзакции (Commit). Система подтверждает, что все предыдущие шаги выполнены успешно, и все изменения (новый баланс) окончательно записываются в базу данных. Блокировки со счёта снимаются.

**Действие 6.** Фиксация финансового результата и уведомление. После успешного коммита система отправляет пользователю уведомление об успешном пополнении.

**2.1. Пополнение через автоплатёж***

Действия при автоплатеже:

**Действие 1. Запуск по расписанию.** Планировщик задач (например, Cron) запускает процесс обработки всех активных автоплатежей на текущую дату.

**Действие 2. Начало транзакции для конкретного автоплатежа.**

**Действие 3. Проверка условий автоплатежа.** Система проверяет, активен ли ещё автоплатёж, не исчерпал ли он лимиты (например, дату окончания), и привязан ли к нему соответствующий платёжный инструмент. Счёт блокируется.

**Действие 4. Списание средств.** Происходит попытка списания через платёжный шлюз. При неудаче система не просто откатывает транзакцию, а обновляет статус автоплатежа (например, на "Ошибка") и, возможно, увеличивает счётчик неудачных попыток. Это нужно, чтобы не пытаться бесконечно списывать средства с проблемной карты.

**Действие 5. Зачисление средств и фиксация (в случае успеха).** Если списание успешно, средства зачисляются на счёт, а запись об автоплатеже обновляется (дата последнего успешного выполнения).

**Действие 6. Логирование и уведомление (вне транзакции).** Независимо от результата, действие логируется. Пользователю отправляется уведомление: "Автоплатёж выполнен успешно" или "Не удалось выполнить автоплатёж. Проверьте данные карты".
